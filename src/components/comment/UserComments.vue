<template>
  <div class="comments">
    ...
    <div v-if="userData.currentUser && userData.userId != userId" class="input-area">
    ...
    <div class="items">
      <CommentItem v-for="(comment, i) in comments" :comment="comment" :key="i" @delete="deleteComment(comment._id)" :can-delete-all="userData.isPremium && userData.userId == userId"></CommentItem>
...
export default {
  ...
  methods: {
    async loadMoreComment() {
      ...
      const PAGE_SIZE = 20;
      socket.emit("user:current-user:comment:list", {
        userId: this.userId,
    ...
    async sendComment() {
      ...
      socket.emit("user:current-user:comment:create", {
        userId: this.userId,
    ...
    async deleteComment(_id) {
      ...
      socket.emit("user:current-user:comment:delete", {
        commentId: _id
      }, (i) => {
        if (!i.ok) {
          this.$vs.notification({
  ...
  created() {
    ...
    socket.on("user:comment", ({ data }) => {
      if (this.userId != data.targetUserId) return;
      this.extraCommentOffset++;
      this.comments.unshift({ ...data, createdAt: Date.now() });
    });

    socket.on("user:comment:delete", ({ data }) => {
