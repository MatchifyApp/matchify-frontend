...
export default {
  ...
  methods: {
    async loadMoreComment() {
      ...
      const PAGE_SIZE = 20;
      socket.emit("album:current-user:comment:list", {
    ...
    async sendComment() {
      ...
      socket.emit("album:current-user:comment:create", {
        albumId: this.albumId,
    ...
    async deleteComment(_id) {
      ...
      socket.emit("album:current-user:comment:delete", {
        commentId: _id
      }, (i) => {
        if (!i.ok) {
          this.$vs.notification({
  ...
  created() {
    ...
    socket.on("album:comment", ({ data }) => {
      if (this.albumId != data.albumId) return;
      this.extraCommentOffset++;
      this.comments.unshift({ ...data, createdAt: Date.now() });
    });

    socket.on("album:comment:delete", ({ data }) => {
