...
export default {
  ...
  methods: {
    async loadMoreComment() {
      ...
      const PAGE_SIZE = 20;
      socket.emit("track:current-user:comment:list", {
    ...
    async sendComment() {
      ...
      socket.emit("track:current-user:comment:create", {
        trackId: this.trackId,
    ...
    async deleteComment(_id) {
      ...
      socket.emit("track:current-user:comment:delete", {
        commentId: _id
      }, (i) => {
        if (!i.ok) {
          this.$vs.notification({
  ...
  created() {
    ...
    socket.on("track:comment", ({ data }) => {
      if (this.trackId != data.trackId) return;
      this.extraCommentOffset++;
      this.comments.unshift({ ...data, createdAt: Date.now() });
    });

    socket.on("track:comment:delete", ({ data }) => {
